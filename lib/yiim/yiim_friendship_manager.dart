...
import 'package:yichat/yiim/yiim_sdk_plugin.dart';
...
class YIIMFriendshipManager {
  ...
  Future<V2TimValueCallback<List<V2TimFriendInfo>>> getFriendList() async {
    ...
    var user = await YiImSDKPlugin.yiIMManager.getLoginUser();
    String? userID = user.data;
    final url = Uri.parse('http://127.0.0.1:8080/users/$userID/friends');
    ...
    if (response.statusCode == 200) {
      ...
      V2TimValueCallback<List<V2TimFriendInfo>> result =
          V2TimValueCallback.fromJson(data);
      return result;
    } else {
      // 请求失败，抛出异常或返回错误信息
      return V2TimValueCallback(
          code: response.statusCode, desc: "getFriendList请求失败");
    }
  }
  ...
  Future<V2TimValueCallback<List<V2TimFriendInfoResult>>> getFriendsInfo({
    ...
    var user = await YiImSDKPlugin.yiIMManager.getLoginUser();
    final url = Uri.parse('http://127.0.0.1:8080/users/friendsInfo');
    final response = await http.post(url,
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({"user_ids": userIDList, "sender": user.data}));
    if (response.statusCode == 200) {
      // 请求成功，解析响应数据
      final data = jsonDecode(response.body);
      ...
      // 请求失败，抛出异常或返回错误信息
      return V2TimValueCallback(
          code: response.statusCode, desc: "getFriendsInfo请求失败");
    }
  ...
  Future<V2TimValueCallback<List<V2TimFriendCheckResult>>> checkFriend({
    ...
    // );
    final url = Uri.parse('http://127.0.0.1:8080/users/user1/checkFriend');
    final response = await http.post(url,
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode(
            {"userIDList": userIDList, "checkType": checkType.index}));
    if (response.statusCode == 200) {
      // 请求成功，解析响应数据
      final data = jsonDecode(response.body);
      V2TimValueCallback<List<V2TimFriendCheckResult>> result =
          V2TimValueCallback.fromJson(data);
      return result;
    } else {
      // 请求失败，抛出异常或返回错误信息
      return V2TimValueCallback(
          code: response.statusCode, desc: "checkFriend请求失败");
    }
  }
  ...
  Future<V2TimValueCallback<V2TimFriendApplicationResult>>
    ...
    final url = Uri.parse('http://127.0.0.1:8080/users/user1/friend-requests');
    final response = await http.get(
      url,
      headers: {'Content-Type': 'application/json'},
    );
    if (response.statusCode == 200) {
      // 请求成功，解析响应数据
      final data = jsonDecode(response.body);
      V2TimValueCallback<V2TimFriendApplicationResult> result =
          V2TimValueCallback.fromJson(data);
      return result;
    } else {
      // 请求失败，抛出异常或返回错误信息
      return V2TimValueCallback(
          code: response.statusCode, desc: "getFriendApplicationList请求失败");
    }
  }
  ...
  Future<V2TimValueCallback<List<V2TimFriendInfo>>> getBlackList() async {
    ...
    final url = Uri.parse('http://127.0.0.1:8080/users/user1/blacklist');
    final response = await http.get(
      url,
      headers: {'Content-Type': 'application/json'},
    );
    if (response.statusCode == 200) {
      ...
      // 请求失败，抛出异常或返回错误信息
      return V2TimValueCallback(
          code: response.statusCode, desc: "getBlackList请求失败");
    }
  }
