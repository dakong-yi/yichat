...
class YIIMConversationManager {
  ...
  Future<void> removeConversationListener(
      {V2TimConversationListener? listener}) async {}
  ...
  Future<V2TimValueCallback<V2TimConversationResult>> getConversationList({
    ...
    final url = Uri.parse('http://127.0.0.1:8080/conversation/users/$userID');
    ...
    if (response.statusCode == 200) {
      ...
      return V2TimValueCallback(code: response.statusCode, desc: 'fail');
      // 请求失败，抛出异常或返回错误信息
    }
  }
  ...
  Future<V2TimValueCallback<int>> getTotalUnreadMessageCount() async {
    ...
    String? userID = user.data;
    final url =
        Uri.parse('http://127.0.0.1:8080/conversation/totalUnread/$userID');
    final response = await http.get(
      url,
      headers: {'Content-Type': 'application/json'},
    );
    if (response.statusCode == 200) {
      // 请求成功，解析响应数据
      final data = jsonDecode(response.body);
      V2TimValueCallback<int> result = V2TimValueCallback.fromJson(data);
      return result;
    } else {
      return V2TimValueCallback(
          code: response.statusCode, desc: "getTotalUnreadMessageCount请求失败");
      // 请求失败，抛出异常或返回错误信息
    }
  ...
  Future<V2TimValueCallback<V2TimConversation>> getConversation({
    ...
    String? userId = user.data;
    final url =
        Uri.parse('http://127.0.0.1:8080/conversation/$userId/$conversationID');
    final response = await http.get(
      url,
      headers: {'Content-Type': 'application/json'},
    );
    if (response.statusCode == 200) {
      ...
      // 请求失败，抛出异常或返回错误信息
      return V2TimValueCallback(
          code: response.statusCode, desc: "getConversation请求失败");
    }
    // return ImFlutterPlatform.instance
