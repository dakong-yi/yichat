...
class YIIMMessageManager {
  ...
  Future<V2TimValueCallback<V2TimMsgCreateInfoResult>> createTextMessage(
    ...
    final url = Uri.parse('http://127.0.0.1:8080/message');
    final user = await YiImSDKPlugin.yiIMManager.getLoginUser();
    ...
    return V2TimValueCallback(code: 1, desc: 'fail');
  }
  ...
  Future<V2TimValueCallback<V2TimMessage>> sendMessage({
    ...
    final url = Uri.parse('http://127.0.0.1:8080/message/send');
    ...
    if (response.statusCode == 200) {
      // 请求成功，解析响应数据
      final data = jsonDecode(response.body);
      V2TimValueCallback<V2TimMessage> res = V2TimValueCallback.fromJson(data);
      return res;
    }
    return V2TimValueCallback<V2TimMessage>(code: 1, desc: 'fail');
  }
  ...
  Future<V2TimCallback> markC2CMessageAsRead({
    ...
    final url = Uri.parse('http://127.0.0.1:8080/message/markC2CMessageAsRead');
    var user = await YiImSDKPlugin.yiIMManager.getLoginUser();
    final response = await http.post(url,
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({"ownerID": user.data, "userID": userID}));
    if (response.statusCode == 200) {
      // 请求成功，解析响应数据
      final data = jsonDecode(response.body);
      V2TimCallback res = V2TimCallback.fromJson(data);
      return res;
    }
    return V2TimCallback(code: -1, desc: '');
  }
  ...
  Future<V2TimValueCallback<List<V2TimMessage>>> getHistoryMessageList({
    ...
    // );
    var user = await YiImSDKPlugin.yiIMManager.getLoginUser();
    final url = Uri.parse('http://127.0.0.1:8080/message/history');
    final response = await http.post(url,
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          "getType": getType!.index,
    ...
    if (response.statusCode == 200) {
      ...
      // 请求失败，抛出异常或返回错误信息
      return V2TimValueCallback(code: response.statusCode, desc: "");
      // throw Exception(
      //     'getHistoryMessageListWithComplete请求失败：${response.statusCode}');
    }
  }
  ...
  Future<V2TimValueCallback<V2TimMessageListResult>> getHistoryMessageListV2({
    ...
    // );
    var user = await YiImSDKPlugin.yiIMManager.getLoginUser();
    final url = Uri.parse('http://127.0.0.1:8080/message/history/v2');
    final response = await http.post(url,
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          "getType": getType!.index,
    ...
    if (response.statusCode == 200) {
      // 请求成功，解析响应数据
      final data = jsonDecode(response.body);
      V2TimValueCallback<V2TimMessageListResult> res =
          V2TimValueCallback.fromJson(data);
      return res;
    } else {
      // 请求失败，抛出异常或返回错误信息
